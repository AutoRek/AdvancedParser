...
namespace ApiSoftware.Library35.Parsing
{
	...
	/// </remarks>
	public sealed class Choice : RuleListBase
	{
		/// <summary>
		...
		public override OutputNode Parse(int position)
		{
			...
			{
				result = item.Parse(position);
				if (result.IsMatch) { return result; }
				if (best == null || result.Begin > best.Begin) { best = result; }
			}
			return best;
			// no choice matched, so return no match with position unchanged
			//var errorNode = new ErrorNode(this, position);
			//Grammar.ErrorNode = errorNode;
			//return errorNode;
		}

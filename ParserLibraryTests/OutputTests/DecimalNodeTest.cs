...
namespace ParserLibraryTests
{
	...
	/// <summary>
	/// This is a test class for DecimalNodeTest and is intended
	/// to contain all DecimalNodeTest Unit Tests
	/// </summary>
	[TestClass()]
	public class DecimalNodeTest : NodeBaseTestClass
	{
		...
		public override void ConstructorTest()
		{
			...
			var index = 1000;
			var node = new DecimalNode(rule, text, index, 100);
		...
		public override void ValueTest()
		{
			...
			Assert.AreEqual(1234.56M, node.Value);
		}
		...
		{
			var node = CreateTestNode();
			Assert.AreEqual("1,234.56", node.NodeText);
		}
		...
		{
			var node = CreateTestNode();
			Assert.AreEqual("[1,234.56]<1,234.56>", node.FormattedOutput());
		}
		...
		public override void FillTest()
		{
			...
			Assert.AreEqual(1234.56M, ds.Tables[0].Rows[0][0]);
		}


		private static DecimalNode CreateTestNode()
		{
			var text = "1,234.56";
			...
			var node = new DecimalNode(rule, text, 0, 8);
			return node;
		}
	}
}

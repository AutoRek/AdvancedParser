...
namespace ParserLibraryTests
{
	...
	/// <summary>
	///This is a test class for BackReferenceTest and is intended
	///to contain all BackReferenceTest Unit Tests
	///</summary>
	[TestClass()]
	public class BackReferenceTest : RuleTestsBaseClass
	{
		...
		/// <summary>
		/// A test for BackReference Constructor
		/// </summary>
		[TestMethod()]
		...
		public override void GetFormattedOutputTest()
		{
			...
			// if the BackReference rule has no template, it does not provide formatted output by default
			rule.Template = null;
		...
		{
			var rule = CreateTestRule();
			Assert.AreEqual("TestRule:ApiSoftware.Library35.Parsing.BackReferenceRule", rule.ToString());
		}
		...
		public override void GetErrorTextTest()
		{
			...
			Assert.AreEqual("Error at 'B' (line 0, position 0): expected 'A'.", error);
		}


		private BackReferenceRule CreateTestRule()
		{
			var rule = new BackReferenceRule();
			rule.Name = "TestRule";
			rule.Template = "[{0}]";

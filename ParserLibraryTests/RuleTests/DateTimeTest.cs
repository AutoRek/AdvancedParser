...
namespace ParserLibraryTests
{
	...
	/// <summary>
	///This is a test class for DateTimeTest and is intended
	///to contain all DateTimeTest Unit Tests
	///</summary>
	[TestClass()]
	public class DateTimeTest : RuleTestsBaseClass
	{
		...
		/// <summary>
		///A test for DateTime Constructor
		///</summary>
		[TestMethod()]
		...
		public override void ParseTest()
		{
			...
			result = rule.Parse("20/10/1999 B");
			...
			rule.Pattern = @"[\d/.-]{6}";
			result = rule.Parse("20109 B");
			Assert.IsFalse(result.IsMatch);

			// 4 digits is matched
			result = rule.Parse("201099 B");
			Assert.IsTrue(result.IsMatch);

			// 5 digits is matched because there is no \b at the end of the symbol
			result = rule.Parse("2010991 B");
			Assert.IsTrue(result.IsMatch);
		...
		public override void ParsePositionTest()
		{
			...
			result = rule.Parse("A 20/10/1999 B", 2);
			Assert.IsTrue(result.IsMatch);
			Assert.AreEqual(new DateTime(1999, 10, 20), result.Value);
		...
		{
			var rule = CreateTestRule();
			var result = rule.Parse("20/10/1999");
			var output = result.FormattedOutput();
			Assert.AreEqual("[19991020]", output);
		}
		...
		{
			var rule = CreateTestRule();
			Assert.AreEqual("TestRule:ApiSoftware.Library35.Parsing.DateTimeRule", rule.ToString());
		}
		...
		public override void InitializeTest()
		{
			...
			// No errors mean pass as DateTime rule is not changed on initialise.
		}
		...
		public void GetErrorReasonTest()
		{
			...
			Assert.AreEqual("Error at 'B' (line 0, position 1): expected a date value.", error);
		}

		private DateTimeRule CreateTestRule()
		{
			var rule = new DateTimeRule();
			rule.Name = "TestRule";
			rule.Template = "[{0:yyyyMMdd}]";
			return rule;
		}
	}
}

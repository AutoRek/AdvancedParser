...
namespace ParserLibraryTests
{
	...
	/// <summary>
	///This is a test class for SaveTest and is intended
	///to contain all SaveTest Unit Tests
	///</summary>
	[TestClass()]
	public class SaveTest : RuleTestsBaseClass
	{
		...
		public override void GetFormattedOutputTest()
		{
			...
			// if the save rule has no template, it does not provide formatted output by default
			rule.Template = null;
		...
		{
			var rule = CreateTestRule();
			Assert.AreEqual("TestRule:ApiSoftware.Library35.Parsing.SaveRule(A)", rule.ToString());
		}

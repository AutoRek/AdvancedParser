...
namespace ParserLibraryTests
{
	...
	/// <summary>
	/// This is a test class for UntilTest and is intended
	/// to contain all UntilTest Unit Tests
	/// </summary>
	[TestClass()]
	public class UntilTest : RuleTestsBaseClass
	{
		...
		{
			var rule = CreateTestRule();
			var result = rule.Parse("CBA");
			var output = result.FormattedOutput();
			Assert.AreEqual("[CB]", output);

			// if the until rule has no template, it does not provide formatted output by default
			rule.Template = null;
		...
		{
			var rule = CreateTestRule();
			Assert.AreEqual("TestRule:ApiSoftware.Library35.Parsing.UntilRule(A)", rule.ToString());
		}
		...
		public override void GetValueTest()
		{
			...
			result = rule.Parse("CBA");
			Assert.AreEqual("CB", rule.GetValue(result));
		...
		public override void GetErrorTextTest()
		{
			...
			Assert.AreEqual("Error at 'B' (line 0, position 0): expected until matching regex pattern 'A'.", error);
		}


		private UntilRule CreateTestRule()
		{
			var rule = new UntilRule("A");
			rule.Name = "TestRule";
			rule.Template = "[{0}]";
			return rule;
		}
